...
const ButtonChild = memo(({ onClick, label }: { onClick: () => void; label: string }) => {
  console.log(`❌ BadUseCallback - ButtonChild "${label}" re-rendered!`);
  return (
    <button
      onClick={onClick}
...
const BadUseCallback = () => {
  const [count, setCount] = useState(0);
  const [text, setText] = useState("");

  // ❌ Function được tạo mới MỖI LẦN render
  // -> memo của ButtonChild vô dụng vì props.onClick luôn khác
  const handleClick = () => {
    setCount((c) => c + 1);
  };
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-xs text-gray-400 mt-2">
        👆 Gõ text và xem console - Buttons re-render dù không liên quan!
      </p>
    </div>
  );
};

export default BadUseCallback;
